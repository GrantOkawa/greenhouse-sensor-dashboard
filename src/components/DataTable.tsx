...
const getMetricColor = (value: number, type: 'temp' | 'humidity' | 'aqi'): string => {
  const colorRanges = {
    temp: {
      good: { min: 20, max: 26 },
  ...
  if (value >= range.good.min && value <= range.good.max) return 'text-green-400';
  if (value >= range.warning.min && value <= range.warning.max) return 'text-yellow-300';
...
const TableRow = memo<{ data: SensorData }>(({ data }) => (
  <tr 
    className="table-row border-b border-white-10 hover:bg-white-5"
  >
    <td className="table-cell font-mono">{formatTimestamp(data.timestamp)}</td>
    <td className="table-cell">{data.sensorId}</td>
    <td className={`table-cell text-right font-semibold ${getMetricColor(data.temperature, 'temp')}`}>
      {data.temperature.toFixed(2)} °C
    </td>
    <td className={`table-cell text-right font-semibold ${getMetricColor(data.humidity, 'humidity')}`}>
      {data.humidity.toFixed(2)} %
    </td>
    <td className={`table-cell text-right font-semibold ${getMetricColor(data.airQuality, 'aqi')}`}>
...
const MobileCard = memo<{ data: SensorData }>(({ data }) => (
  <div className="mobile-card bg-white-10 border border-white-20">
    <div className="flex justify-between items-center mb-2">
      ...
      <span className="text-sm text-gray-300 font-mono">{formatTimestamp(data.timestamp)}</span>
    </div>
    <div className="mobile-data-grid">
      <div>
        <p className="metric-label">Temp</p>
        <p className={`metric-value ${getMetricColor(data.temperature, 'temp')}`}>
          {data.temperature.toFixed(1)}°
        </p>
      </div>
      <div>
        <p className="metric-label">Humidity</p>
        <p className={`metric-value ${getMetricColor(data.humidity, 'humidity')}`}>
        ...
        <table className="table">
          <thead className="table-header text-xs text-white uppercase">
            <tr>
              ...
              <th scope="col" className="table-header-cell">Sensor ID</th>
              <th scope="col" className="table-header-cell text-right">Temperature</th>
              <th scope="col" className="table-header-cell text-right">Humidity</th>
              <th scope="col" className="table-header-cell text-right">Air Quality</th>
            </tr>
          </thead>
          <tbody>
            {data.map((item) => (
